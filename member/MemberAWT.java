...
//			System.out.println(vlist.size());
		...
		for (int i = 0; i < vlist.size(); i++) {
			...
			String str = (i + 1)/* bean.getId() */ + "    " + bean.getName().trim() + "       " + bean.getPhone().trim()
					+ "       " + bean.getTeam();// 데이터 표시 부분
			list.add(str);
		}
		int len = list.getItemCount();
		if (len > 0)
			list.select(0);// 초기 커서 위치 지정
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == b1/* 수정버튼 */) {
		...
		} else if (obj == zipBtn) {//주소버튼
			if (zf == null)
				zf = new ZipcodeFrame(this);
			else
				zf.setVisible(true);
		} else if (obj == insBtn/*추가 입력*/) {
			...
			bean.setTeam(tf3.getText());
			bean.setAddress(tf4.getText());
			if(mgr.update(bean)) {//데이터 수정 후에 초기화 작업
				p3.removeAll();
				remove(p3);
				vlist.removeAllElements();
				viewList();
